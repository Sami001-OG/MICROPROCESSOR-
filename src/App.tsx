      ...
      <main className="flex-1 flex flex-col overflow-hidden relative min-w-0">
        ...
        <header className="h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-4 md:px-8 shrink-0 z-10 transition-colors duration-300">
          <div className="flex items-center gap-3 min-w-0">
            ...
            {view !== 'dashboard' && (
              <div className="hidden sm:flex bg-slate-100 dark:bg-slate-800 p-1 rounded-lg ml-2 shrink-0">
                <button 
          ...
          <div className="flex items-center gap-3 md:gap-4 shrink-0">
            ...
            <div className="hidden sm:flex items-center gap-2">
              <div className="w-24 md:w-32 h-2 bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
                <motion.div 
                  initial={{ width: 0 }}
              ...
              <span className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-tighter">{progressPercent}%</span>
            </div>
            <button 
                  ...
                  {curriculum.map((module) => (
                    <div key={module.id} className="tech-card group hover:border-slate-400 dark:hover:border-slate-600 transition-all flex flex-col">
                      <div className="p-6 md:p-8 flex-1 flex flex-col">
                        ...
                        <p className="text-slate-500 dark:text-slate-400 mb-6 md:mb-8 leading-relaxed text-sm md:text-base">{module.description}</p>
                        
                        <div className="space-y-3 mb-8 flex-1">
                          {module.lessons.slice(0, 4).map((lesson) => (
                            <div key={lesson.id} className="flex items-center justify-between text-sm text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-800/50 p-2 rounded-lg">
                              <div className="flex items-center gap-2 truncate pr-4">
                                <BookOpen size={14} className="text-slate-400 dark:text-slate-500 shrink-0" />
                                <span className="truncate">{lesson.title}</span>
                              </div>
                              {completedLessons.includes(lesson.id) && <CheckCircle2 size={14} className="text-green-500 dark:text-green-400 shrink-0" />}
                            </div>
                          ))}
                          {module.lessons.length > 4 && (
                            <div className="text-xs text-slate-400 dark:text-slate-500 font-medium text-center pt-2">
